 ...
 */
function checkPageBreak(doc, space = 120) {
...
function renderOrdersPdf(doc, orders, periodo) {
    ...
    doc.moveDown(0.5);
    doc.font("Helvetica").fontSize(10).fillColor("#555").text(`Período: ${periodo}`, { align: "center" });
    ...
    orders.forEach((order, index) => {
        ...
        doc
            ...
            .fontSize(9)
            .fillColor("#444")
            .text(`Mesa: ${order.mesa ?? "-"}   |   Tipo: ${order.tipo ?? "-"}   |   Status: ${order.status}`);
        resetX(doc);
        ...
        order.pedido.forEach((item) => {
            ...
            doc.text(`R$ ${(item.price * item.quantity).toFixed(2)}`, 460, doc.y - 12, { width: 80, align: "right" });
            doc.moveDown(0.3);
        });
        doc.moveDown(0.5);
        resetX(doc);
...
//# sourceMappingURL=orderPdfLayout.js.map